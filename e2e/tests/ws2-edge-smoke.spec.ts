...
test('edges: trio — leaver rates BOTH partners, survivors keep talking, departed rated at round end', async () => {
  ...
  // Rate partner 1 (star + submit), then partner 2's form must follow.
  const rateOne = async (page: Page) => {
    ...
    await page.waitForTimeout(1500); // confirmation auto-advances
  };
  await rateOne(leaverPage);
  await expect(leaverPage.getByText(/How was your chat with/i).first(), 'leaver gets a SECOND partner form (trio)')
    .toBeVisible({ timeout: 10_000 });
  console.log('  ✓ leaver rates BOTH partners (sequential forms)');
  await rateOne(leaverPage);
  ...
  if (isAlgorithmTrio) {
    ...
    await expect(s0Page.getByText(/How was your chat with/i).first(),
      ...
      .toBeVisible({ timeout: 10_000 });
    console.log('  ✓ departed member included in the survivor’s round-end rating (browser-proven)');
  } else {

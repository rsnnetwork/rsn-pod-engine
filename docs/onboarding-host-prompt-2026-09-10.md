...
Two things are enforced by the server, not left to the model: at most six host questions in the whole chat (three openings and at most one follow-up after each, Claus's shape), and two short answers in a row (four words or fewer, once a second question has been asked) end the questions. If the model still asks instead of closing, the server asks it for the closing only and, failing that, closes the turn itself.
...
- Ask at most six questions in the whole chat, counting the opening: the three openings and at most one follow-up after each. A garbled or unclear answer is not a reason for another question; take what you can from it and move on. So far you have asked 1 of at most 6.
...
When they press it twice, reach six questions, or answer briefly twice in a row:

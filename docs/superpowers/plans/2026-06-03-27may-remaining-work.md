...
**Last updated:** 2026-06-04 (Ship B shipped — `087ba44`)
...
### Workstream 1 — Canonical migration to 100% (Ships A `66f4892` + B `087ba44` DONE; next = Ship C, back-to-back per Ali 2026-06-04)
1. **Ship C (next):** strip tokens from match:assigned/reassigned + retire lobby:token (events stay as lifecycle notifications); client connects purely via snapshot you.token + REST fallback; update token-asserting tests (e.g. `phase-may19-realtime-migration-phase4.test.ts:209`); full regression sweep on manual breakouts + reconnect + trio flows. Full design in the approved plan (plans/ok-i-want-you-humble-locket.md mirror) + specs/2026-05-25-canonical-room-state-design.md §8. NOTE: Ali confirmed the A-test gate 2026-06-04 ("seems fine") and approved B+C back-to-back; he has ONE Ship A issue to report after B ships — triage it BEFORE starting C.
2. **Phase 3 completion — flip remaining legacy in-memory read paths to canonical** (lobby/breakout rosters + misc reads still hit in-memory maps/DB). In-memory maps become cache only, rebuilt from Redis on boot. (Partially advanced by Ship B: all presence-membership gates now read canonical-first.)
